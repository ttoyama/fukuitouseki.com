...
├── _quarto_booklet.yml ★PDF生成設定ファイル★ (統合PDF用設定)
├── make_pdf.sh ★PDF生成スクリプト★ (ワンコマンドPDF生成)
├── 
├── docs/ ★コンテンツフォルダ★ (全QMDファイルがここに配置)
│   ├── ★マニュアル本体★
│   ├── 0100_各施設の役割と対応.qmd
│   ├── 0200_福井県透析施設ネットワークの役割と対応.qmd
│   │
│   ├── ★平時準備チェックシート群（13xx）★
│   ├── 1310_施設設備点検チェックシート.qmd
│   ├── 1311_施設設備点検表.qmd
│   ├── 1320_備蓄物資点検チェックシート.qmd
│   ├── 1321_物資薬剤管理チェックシート.qmd
│   ├── 1322_物資薬剤備蓄状況確認表.qmd
│   ├── 1330_患者緊急連絡先チェックシート.qmd
│   │
│   ├── ★初動対応チェックシート群（14xx）★
│   ├── 1410_初動対応チェックシート_0-30分.qmd
│   ├── 1420_初動対応チェックシート_1-3時間.qmd
│   ├── 1430_初動対応チェックシート_3-24時間.qmd
│   ├── 1431_透析実施困難施設対応チェックシート.qmd
│   ├── 1440_災害時透析医療機関状況報告書_初動報告用.qmd
│   ├── 1441_災害時透析医療機関状況報告書_詳細報告用.qmd
│   ├── 1442_初動対応記録表.qmd
│   ├── 1450_スタッフ安否確認表.qmd
│   ├── 1451_患者安全確認表.qmd
│   ├── 1452_患者安全・スタッフ安否チェックシート.qmd
│   ├── 1453_患者カテゴリー別優先度評価表.qmd
│   ├── 1460_透析関連機器確認表.qmd
│   ├── 1461_施設設備点検チェックシート_発災直後.qmd
│   ├── 1470_交通アクセス状況チェックシート.qmd
│   │
│   ├── ★ネットワーク支援体制チェックシート群（23xx）★
│   ├── 2310_支援準備状況チェックシート.qmd
│   │
│   ├── ★ネットワーク対応チェックシート群（24xx）★
│   ├── 2410_被災地情報集約チェックシート.qmd
│   ├── 2411_初動調整チェックシート.qmd
│   ├── 2420_緊急搬送調整チェックシート.qmd
│   ├── 2421_患者搬送調整チェックシート.qmd
│   ├── 2430_患者受入調整チェックシート.qmd
│   ├── 2440_二次避難計画チェックシート.qmd
│   ├── 2441_二次避難患者受入チェックシート.qmd
│   │
│   ├── ★参考資料群（99xx）★
│   ├── 9910_公的防災情報リンク集.qmd
│   ├── 9920_福井県内透析施設一覧.qmd
│   ├── 9930_透析患者向け食品栄養成分表.qmd
│   │
│   └── custom.css ★スタイル設定★
│
├── booklet-pdf/ ★PDF出力先★
│   ├── archived/ ★過去バージョン保管★
│   └── fukuitouseki_booklet_YYYYMMDD_HHMM.pdf ★最新PDF★
│
├── _site/ ★Webサイト出力先★ (Quartoレンダリング先)
...
【重要な変更点】
- フォルダ構造は廃止され、全QMDファイルがdocs/直下にフラット配置
- ファイル名の先頭番号で種類と順序を管理：
  - 01xx: マニュアル本体
  - 13xx: 平時準備チェックシート
  - 14xx: 初動対応チェックシート  
  ...
  - 24xx: ネットワーク対応
  - 99xx: 参考資料
- 全34のQMDファイルが個別のチェックシート・様式として機能
- PDF生成は make_pdf.sh で全ファイルを統合して生成
