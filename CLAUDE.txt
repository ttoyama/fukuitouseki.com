...
```
fukuitouseki.com/
├── .github/                    # GitHub Actions（自動PDF生成）
├── .bookorder                  # PDF生成時のファイル順序定義
├── CLAUDE.md                   # Claude Code作業ガイドライン
├── README.md                   # プロジェクト概要
├── _config.yml                 # Jekyllサイト設定
├── index.md                    # サイトトップページ
├── booklet-pdf/                # PDF生成管理
│   ├── archived/               # 古いPDFのアーカイブ
│   └── fukuitouseki_booklet_YYYYMMDD_HHMM.pdf  # 最新PDF
├── docs/                       # メインコンテンツ
│   ├── 000_マニュアル本体/     # Markdown版マニュアル
│   │   ├── 01-各施設の役割と対応.md
│   │   └── 02-福井県透析施設ネットワークの役割と対応.md
│   ├── 101-119_初動対応系/     # 0-24時間対応チェックシート・様式
│   ├── 211-253_平時準備系/     # 平時の準備チェックシート
│   ├── 311-363_調整・搬送系/   # ネットワーク調整・患者搬送関連
│   ├── 424-563_支援系/         # 支援準備・二次避難関連
│   ├── 999_資料/              # 参考資料（施設一覧・栄養成分表等）
│   └── index.md                # ドキュメント目次
...
│   │   └── 静岡/
│   └── 全腎協マニュアル/
└── docx_to_markdown.py         # Word→Markdown変換スクリプト
```
...
- **チェックシート・様式集** (`docs/1XX-5XX_`): LaTeX形式の実用的な記録様式
  - **初動対応系** (101-119): 災害発生0-24時間の対応チェックシート
  - **平時準備系** (211-253): 平時の点検・準備チェックシート
  - **調整・搬送系** (311-363): ネットワーク調整・患者搬送関連
  - **支援系** (424-563): 支援準備・二次避難関連
